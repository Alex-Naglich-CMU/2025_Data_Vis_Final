<script lang="ts">
    ...
    let { drugsData }: { drugsData: DrugData[] } = $props();
    ...
    let oldPrice = $derived(
        insulinData && insulinData.prices.length > 0 
            ? Math.round(insulinData.prices[0].price) 
            : 0
    );
...
<div class="comparison-container">
    ...
    <p class="note">{Math.abs(percentChange)}% {percentChange < 0 ? 'decrease' : 'increase'} in price</p>
</div>
